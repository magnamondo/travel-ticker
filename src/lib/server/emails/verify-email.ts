...
export async function sendVerifyEmail(data: VerifyEmailData, origin: string, db?: Database) {
	const logoUrl = `${origin}/logo.jpg`;
	const verificationUrl = `${origin}/verify-email?token=${data.verificationToken}`;
...
<body style="margin: 0; padding: 0; background-color: #ffffff; color: #000000; font-family: system-ui, -apple-system, sans-serif;">
		...
		<tr>
			<td align="center" style="padding: 40px 20px;">
				...
				<div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; padding: 40px; text-align: center;">
					...
					<h1 style="color: #000000; margin: 0 0 20px; font-size: 24px;">Welcome aboard!</h1>
					
					<p style="color: #333333; font-size: 16px; line-height: 1.6; margin: 0 0 30px;">
						We're excited to have you join us. Please verify your email address to unlock your account and get started.
					</p>
					
					<div style="margin-bottom: 30px;">
						<a href="${verificationUrl}" style="background-color: #000000; color: #ffffff; text-decoration: none; padding: 12px 32px; border-radius: 4px; font-weight: 600; display: inline-block; font-size: 16px; transition: opacity 0.2s;">
							Verify Email
						</a>
					</div>
	...
	return sendEmail(
		{
			to: data.email,
			subject: 'Verify your email - Magnamondo',
			html
		}
	);
}
