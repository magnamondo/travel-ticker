...
<svelte:head>
	<title>{data.milestone.title} | {data.ticker.name} | Travel Ticker | Magnamondo</title>
	...
	<meta property="og:url" content="{data.origin}/t/{data.ticker.slug}/entry/{data.milestone.id}">
...
<div class="entry-page">
	<a href={resolve(`/t/${data.ticker.slug}`)} class="back-link">
		<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			<path d="M19 12H5M12 19l-7-7 7-7"/>
		</svg>
		Back to {data.ticker.name}
	</a>
	...
	<section class="comments-section">
		...
		{:else}
			<div class="login-prompt">
				<p>Please <a href={`${resolve('/login')}?redirectTo=${encodeURIComponent(`/t/${data.ticker.slug}/entry/${data.milestone.id}`)}`}>log in</a> to leave a comment.</p>
			</div>
		{/if}
