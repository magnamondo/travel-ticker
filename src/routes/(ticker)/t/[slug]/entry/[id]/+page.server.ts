...
import { milestone, segment, milestoneMedia, comment, reaction, userProfile, user, ticker } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ params, locals, url }) => {
	...
	// The entry lives under exactly one ticker; a mismatched slug is a stale or
	// hand-edited URL, so send the visitor to the canonical one.
	if (milestoneResult.tickerSlug !== params.slug) {
		throw redirect(301, `/t/${milestoneResult.tickerSlug}/entry/${milestoneResult.id}`);
	}

	if (!milestoneResult.tickerPublished && !canViewUnpublishedTickers(locals.user?.roles)) {
		throw error(404, 'Entry not found');
	}
	...
	return {
		origin: url.origin,
		ticker: {
			slug: milestoneResult.tickerSlug,
			name: milestoneResult.tickerName,
			published: milestoneResult.tickerPublished
		},
		milestone: {
...
export const actions: Actions = {
	default: async ({ request, params, locals }) => {
		if (!locals.user) {
			throw redirect(
				303,
				`/login?redirectTo=${encodeURIComponent(`/t/${params.slug}/entry/${params.id}`)}`
			);
		}
