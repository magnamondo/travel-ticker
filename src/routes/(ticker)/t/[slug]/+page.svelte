<script lang="ts">
	...
	function formatReactions(
		grouped: Record<string, { count: number; userIds: string[] }>,
		userId?: string
	): ReactionCount[] {
	...
	async function loadMore() {
		...
		try {
			const response = await fetch(
				`/api/milestones?ticker=${encodeURIComponent(data.ticker.slug)}&offset=${milestones.length}&limit=3`
			);
	...
	let socialDescription = $derived(
		data.ticker.description ?? data.ticker.tagline ?? `Follow along on ${data.ticker.name}.`
	);
	let socialImage = $derived(
		data.ticker.coverImage ? `${data.origin}${data.ticker.coverImage}` : `${data.origin}/logo.png`
	);
	let canonicalUrl = $derived(`${data.origin}/t/${data.ticker.slug}`);
</script>
...
<section class="timeline-section" bind:this={sectionEl}>
	...
	>
		...
		{#if data.ticker.originLabel && data.ticker.destinationLabel}
			<p>
				<span>{data.ticker.originLabel}</span>
				...
				</svg> <span>{data.ticker.destinationLabel}</span>
			</p>
		{:else}
			<p class="ticker-name-line"><span>{data.ticker.name}</span></p>
		{/if}
		{#if data.ticker.tagline && (data.ticker.originLabel || data.ticker.destinationLabel)}
			<p class="ticker-tagline">{data.ticker.tagline}</p>
		{/if}
		{#if !data.ticker.published}
			<p class="ticker-draft-badge">📝 Draft ticker - only visible to admins</p>
		{/if}
	</header>
		...
		<div class="segment-section">
			...
			<div class="timeline">
				{#each group.milestones as milestone, idx (milestone.id)}
					...
					{@const isNewDate =
						!prevMilestone || getDateKey(milestone.date) !== getDateKey(prevMilestone.date)}
					{#if isNewDate}
					...
					<div
						...
						style={swipingItemId === milestone.id
							? `--swipe-offset: ${getSwipeOffset(milestone.id)}px`
							: ''}
					>
						{#if milestone.meta?.length}
							<div
								class="timeline-meta"
								class:meta-left={milestone.side === 'right'}
								class:meta-right={milestone.side === 'left'}
							>
								{#each milestone.meta as meta (meta.type + meta.value)}
									{#if meta.type === 'coordinates'}
										<a
											href={getMapsUrl(meta.value)}
											target="_blank"
											rel="noopener"
											class="meta-item meta-coordinates"
										>
						...
						<div class="timeline-content">
							<a
								href={resolve(`/t/${data.ticker.slug}/entry/${milestone.id}`)}
								style="text-decoration: none; color: inherit; display: block;"
							>
								<div class="card">
									{#if !milestone.published}
										<div class="draft-badge" title="Draft - not visible to others">📝 Draft</div>
									{/if}
									{#if milestone.groupNames?.length}
										<div class="group-badge" title={milestone.groupNames.join(', ')}>
											🔒 {milestone.groupNames.length === 1
												? milestone.groupNames[0]
												: `${milestone.groupNames.length} groups`}
										</div>
									{/if}
									...
									<div class="card-body" class:text-only={!milestone.media?.length}>
										...
										{#if milestone.media}
											{@const readyMedia = milestone.media.filter(
												(m) => m.type === 'image' || m.thumbnailUrl
											)}
											{@const expanded = isMediaExpanded(milestone.id)}
											{@const maxVisible = 3}
											{@const displayMedia = expanded
												? readyMedia
												: readyMedia.slice(0, maxVisible)}
											...
											{#if readyMedia.length > 0}
												<div class="media-grid" class:expanded>
													{#each displayMedia as item, i (i)}
														{@const mediaIndex = readyMedia
															.slice(0, expanded ? i : i)
															.filter((m) => m.type === 'image' || m.isReady).length}
														{@const isLastVisible =
															!expanded && i === maxVisible - 1 && hiddenCount > 0}
														{#if item.type === 'image'}
															<button
																...
																onclick={(e) =>
																	isLastVisible
																		? toggleMediaExpanded(milestone.id, e)
																		: openLightbox(readyMedia, mediaIndex, e)}
																ontouchstart={(e) => e.stopPropagation()}
																aria-label={isLastVisible
														...
														{:else}
															...
															<button
																...
																onclick={(e) =>
																	isLastVisible
																		? toggleMediaExpanded(milestone.id, e)
																		: item.isReady && openLightbox(readyMedia, mediaIndex, e)}
																ontouchstart={(e) => e.stopPropagation()}
																aria-label={isLastVisible
																	? `Show ${hiddenCount} more`
																	: item.isReady
							...
							<div
								...
								class:hidden={swipedItemId === milestone.id}
								ontouchstart={(e) => e.stopPropagation()}
							>
								<Reactions
...
<style>
	...
	/* Single-name fallback: no arrow, so drop the two-column alignment. */
	.timeline-header p.ticker-name-line span {
		min-width: 0;
		text-align: center;
	}

	.timeline-header p.ticker-name-line {
		font-size: 1rem;
		color: var(--color-text-secondary);
	}

	.timeline-header p.ticker-tagline {
		margin-top: 0.4rem;
		font-size: 0.75rem;
	...
	.timeline-header p.ticker-draft-badge {
		margin-top: 0.75rem;
	...
	/* Matches the entry page's back link: top left, mirroring the user menu
	   at top right, rather than sitting inside the centred header. */
	.back-link {
		display: inline-flex;
		align-items: center;
		gap: 0.5rem;
	...
	@media (max-width: 768px) {
		...
		.timeline-item.swiping .timeline-content .card {
			transition: none;
			box-shadow:
				0 calc(8px * min(1, var(--swipe-offset, 0) / 120))
					calc(25px * min(1, var(--swipe-offset, 0) / 120))
					rgba(0, 0, 0, calc(0.15 * min(1, var(--swipe-offset, 0) / 120))),
				0 calc(4px * min(1, var(--swipe-offset, 0) / 120))
					calc(10px * min(1, var(--swipe-offset, 0) / 120))
					rgba(0, 0, 0, calc(0.1 * min(1, var(--swipe-offset, 0) / 120)));
		}

		.timeline-item.swiping-back .timeline-content .card {
			transition: none;
			box-shadow:
				0 calc(8px * max(0, 1 + var(--swipe-offset, 0) / 120))
					calc(25px * max(0, 1 + var(--swipe-offset, 0) / 120))
					rgba(0, 0, 0, calc(0.15 * max(0, 1 + var(--swipe-offset, 0) / 120))),
				0 calc(4px * max(0, 1 + var(--swipe-offset, 0) / 120))
					calc(10px * max(0, 1 + var(--swipe-offset, 0) / 120))
					rgba(0, 0, 0, calc(0.1 * max(0, 1 + var(--swipe-offset, 0) / 120)));
		}
